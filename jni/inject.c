...
#define LOGD(fmt, args...)  printf(fmt, ##args) // __android_log_print(ANDROID_LOG_DEBUG,LOG_TAG, fmt, ##args)
...
void* get_remote_addr(pid_t target_pid, void* local_addr)
{
    ...
    memset(module_path, 0, 256);  //将新申请的内存块中填充为0
    get_module_path((unsigned long)local_addr, module_path);  //根据某函数在内存中的地址范围找到对应的文件

    local_handle = get_module_base(-1, module_path); //内存基地址
    remote_handle = get_module_base(target_pid, module_path);

    DEBUG_PRINT("[+] get_remote_addr: local[%p], remote[%p]\n", local_handle, remote_handle);
    void * ret_addr = (void *)((uint32_t)local_addr + (uint32_t)remote_handle - (uint32_t)local_handle);  //内存中实际地址
...
int inject_remote_process(pid_t target_pid, const char *injected_lib, const char *function_name, const char *param, size_t param_size)
{
    ...
    /* save original registers */ //保存原始寄存器的状态内容
    ...
    /* call mmap */    
    parameters[0] = 0;  // addr    
    parameters[1] = 1024*100; // size  在目标pid进程中申请的内存空间的大小, 大小根据so文件的大小来给
    parameters[2] = PROT_READ | PROT_WRITE | PROT_EXEC;  // prot    
    parameters[3] = MAP_ANONYMOUS | MAP_PRIVATE; // flags    
    parameters[4] = 0; //fd    
    parameters[5] = 0; //offset    
    
    //调用目标pid进程的mmap函数，在目标pid进程的内存空间中申请内存空间
    if (ptrace_call_wrapper(target_pid, "mmap", mmap_addr, parameters, 6, &regs) == -1)    
        goto exit; //调用失败，跳转

    //获取调用目标pid进程的mmap函数完成后的函数返回值，也就是刚申请内存空间地址
    map_base = (uint8_t *)ptrace_retval(&regs);

    //测试代码：start 尝试在刚开辟的空间内写入字符串测试是否成功
    ...
    //调用ptrace_writedata将so文件内容写入到内存; 从刚通过mmap申请的内存地址(map_base)处开始写入
    ...
    if (sdk_ver > AND60_SDK) {
        ...
        //调用dlopen加载so文件, 为后续使用dlsym函数做准备
        if (ptrace_call_wrapper(target_pid, "dlopen", dlopen_addr, parameters, 2, &regs) == -1)
            goto exit;
    }

    //获取dlopen函数调用后的返回值即injected_lib指定的so文件在目标pid进程中的加载基址
    void* sohandle = (void*)ptrace_retval(&regs);

//设置map_base中保存injected_lib指定的so文件的导出函数function_name字符串的内存偏移
#define FUNCTION_NAME_ADDR_OFFSET 0x100
    ...
    //获取调用dlsym函数后，返回的导出函数function_name的调用地址 并且将地址打印
    void* hook_entry_addr = (void*)ptrace_retval(&regs);    
    DEBUG_PRINT("hook_entry_addr = %p\n", hook_entry_addr);    
    
//设置map_base中保存调用导出函数function_name需要的函数参数的内存偏移
#define FUNCTION_PARAM_ADDR_OFFSET      0x200    
    //将调用hook_entry_addr函数需要的函数参数保存到前面在目标pid进程中mmap申请的内存空间offset=0x200的位置上
    ...
    //调用注入到目标pid进程中的so库的导出函数hook_entry，实现我们自定义的代码，可以是hook 目标pid进程的函数等
    if (ptrace_call_wrapper(target_pid, "hook_entry", hook_entry_addr, parameters, 1, &regs) == -1)    
        goto exit;        

    //从控制台读取输入后进行detach等操作
...
static void init_sdk_ver(){
    ...
    if (len <= 0) {
        LOGD("[init_sdk_ver] read ro.build.version.sdk error!");
        return;
    }
...
int main(int argc, char** argv) {    
    if (argc < 3) {
        //执行该脚本时，注意要将libpath写成绝对路径！否则容易出现dlopen返回地址是0的情况
